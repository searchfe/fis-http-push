...
describe('各种失败场景', () => {
    ...
    it('输入邮箱发生异常', async () => {
        ...
        await expect(push('/bar.txt', '/bar', options)).rejects.toHaveProperty('message', 'Upload file "/bar.txt" to "http://localhost:1080/bar" failed: "intended"');
    });
    ...
    it('并发情况下的错误', () => {
        ...
        return expect(pushMultiple(tasks, opts)).rejects.toHaveProperty('message', 'Upload file "/foo.txt" to "http://localhost:1080/unkown-error" failed: "500 UNKOWN"');
    });
    ...
    it('并发情况下的输入异常', async () => {
        ...
        await expect(pushMultiple(tasks, options)).rejects.toHaveProperty('message', 'Upload file "/foo.txt" to "http://localhost:1080/unkown-error" failed: "intended"');
    });
});
